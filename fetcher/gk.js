function generateMatchupDataWithFullTeamNames() {
    ...
    for (let i = 1; i < sourceData.length; i++) { // Start from row 2 to skip the header
      ...
      // Only process if the position is "gkp" (goalkeeper), case-insensitive
      if (pos.toLowerCase() === "gkp") {
        ...
        otherTeams.forEach(opponent => {
          ...
          rowsToInsert.push([
            ...
            generateHash(playerName + teamName), // Column E: GKP ID (hash of player name + team name)
    ...
    Logger.log("Matchup data generated and inserted successfully.");
  }

function generateMatchupDataForForwards() {
    ...
    Logger.log("Starting data generation for forwards...");
    ...
    for (let i = 1; i < sourceData.length; i++) { // Start from row 2 to skip the header
      ...
      // Only process if the position is "for" (forward), case-insensitive
      if (pos.toLowerCase() === "for") {
        ...
        otherTeams.forEach(opponent => {
          ...
          rowsToInsert.push([
            ...
            generateHash(playerName + teamName), // Column E: FOR ID (hash of player name + team name)
    ...
    Logger.log("Matchup data for forwards generated and inserted successfully.");
  }

document.addEventListener('DOMContentLoaded', function() {
    ...
    teamBoxes.forEach(box => {
        ...
        plusIcon.addEventListener('click', async (e) => {
            ...
            try {
                const response = await fetch('http://localhost/fpl_hub/tvst.php?action=teams');
        ...
        searchInput.addEventListener('input', async (e) => {
            ...
            try {
                const response = await fetch(`http://localhost/fpl_hub/tvst.php?action=search&query=${encodeURIComponent(search)}`);
    ...
    async function updateTeamsList(list, teams, boxIndex) {
        list.innerHTML = '';
        teams.forEach(team => {
    ...
    async function selectTeam(team, boxIndex) {
        try {
            const response = await fetch(`http://localhost/fpl_hub/tvst.php?action=stats&team_id=${team.team_id}`);
    ...
    function compareAndHighlight() {
        ...
        activeBoxes.forEach(boxIndex => {
            ...
            Object.entries({...teamStats.strength, ...teamStats.matches}).forEach(([key, value]) => {
